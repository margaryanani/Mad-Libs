...
def get_prompts(template_mapping, prompts_and_categories, num):
    """Returns a list of prompts with their categories based
     on the selected template number.
    Args:
        template_mapping (dict): A dictionary mapping template numbers (str) to
                                 lists of indices that correspond
                                 to specific prompts.
    ...
    returns:
            list: list of tuples, where each tuple has its prompt
                  and category based on the chosen template.
    """
...
def inputs(prompts, categories):
    """Displays prompts to the user, allowing the user to input words
    for the game, if the user enters '?' a random word is chosen from the
    corresponding category. Prevents blank inputs by prompting the user to
    re-enter a valid input.
    Args:
        prompts (list): contains tuples, where each prompt has its own category.
        categories (dict): contains keys that are the category names and their
                           values are a list of words within that category.
    returns:
